...
def convert_apk_to_pak(apk_path: Path, pak_path: Path):
    ...
    if not offsets:
        print("❌ No se encontraron entradas válidas en el APK.")
        return
    ...
    print(f"✅ Convertido: {pak_path}  ({len(pak_data)} bytes)")

# Entry point
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Uso: python repair.py archivo.apk")
    else:
        apk_file = Path(sys.argv[1])
        if not apk_file.exists():
            print(f"❌ No se encontró el archivo: {apk_file}")
            sys.exit(1)

        output_dir = Path("output")
        output_file = output_dir / (apk_file.stem + ".pak")
        convert_apk_to_pak(apk_file, output_file)
